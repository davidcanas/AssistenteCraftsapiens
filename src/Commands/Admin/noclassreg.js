    ...
    async run(interaction) {
        ...
        if (interaction.data.options[0].value === 'off') {
            ...
            interaction.createMessage({
                content: 'Prontinho! Agora voltarão a haver aulas durante a semana!',
                flags: 1 << 6
            })
        } else {
            if (!interaction.data.options[1]) return interaction.createMessage({
                content: 'Por favor, especifique um motivo para não haver aulas!',
                flags: 1 << 6
            })
            this.client.db.set('noclass', {state: true, reason: interaction.data.options[1].value})
            interaction.createMessage({
                content: 'Prontinho! Agora não haverão aulas durante a semana!\n\nMotivo: `' + interaction.data.options[1].value + '`\n\n`Não te esqueças de usar o comando novamente quando voltarem a haver aulas!`',
                flags: 1 << 6
            })
        }
    }
