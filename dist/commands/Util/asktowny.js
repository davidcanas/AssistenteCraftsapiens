...
function extractText(candidate) {
    if (!candidate?.content?.parts?.length)
        return ""; // fallback se não houver parts
    return candidate.content.parts[0].text || "";
}
class AskTowny extends Command_1.default {
    constructor(client) {
        super(client, {
            name: "asktowny",
    ...
    async execute(ctx) {
        ...
        try {
            ...
            const townyPath = path_1.default.resolve(__dirname, "../../data/config.yml");
            const eventWarPath = path_1.default.resolve(__dirname, "../../data/warconfig.yml");
            const docsPath = path_1.default.resolve(__dirname, "../../data/towny_docs.txt");
            // Ler Config Towny
            if (fs_1.default.existsSync(townyPath)) {
                townyConfigContent = fs_1.default.readFileSync(townyPath, "utf-8");
            }
            else {
                townyConfigContent = "Configuração do Towny (config.yml) não encontrada.";
            }
            // Ler Config EventWar
            if (fs_1.default.existsSync(eventWarPath)) {
                eventWarConfigContent = fs_1.default.readFileSync(eventWarPath, "utf-8");
            }
            else {
            ...
            if (fs_1.default.existsSync(docsPath)) {
                townyDocsContent = fs_1.default.readFileSync(docsPath, "utf-8");
            }
            else {
