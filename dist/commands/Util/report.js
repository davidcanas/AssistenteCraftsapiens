...
const node_fetch_1 = __importDefault(require("node-fetch")); // Certifique-se de importar o fetch se não for global
class ReportCommand extends Command_1.default {
    ...
    async execute(ctx) {
        ...
        if (!ctx.msg.messageReference?.messageID) {
        ...
        const message = await ctx.channel.getMessage(ctx.msg.messageReference?.messageID);
        if (!message) {
        ...
        try {
            ...
            if (result.toLowerCase().includes("[sim]")) {
                ...
                if (channel && channel instanceof oceanic_js_1.TextChannel) {
