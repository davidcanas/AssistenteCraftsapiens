...
class MessageCreate {
    ...
    async run(message) {
        ...
        if (message.member && message.member.nick && !this.client.getDiscordByNick(message.member.nick) && message.content.match(/^\d{4}$/)) {
            ...
            if (!message.channel) {
                message.author.createDM().then(a => a.createMessage({
        ...
        if (message.channel.type === 1)
            return;
        async function checkForLinks(db, phrase) {
                    ...
                    embeds: [
                        {
                            ...
                            footer: {
                                ...
                                iconURL: "https://cdn.discordapp.com/avatars/968686499409313804/9c644fe8502c2dfdf976947c66671749.png?size=4096"
                            },
                            color: 15204352
                        }
                    ]
        ...
        if (!message.content.startsWith(prefix) &&
            ...
            if (dbcheck && dbcheck.ignoredUsers.includes(message.author.id))
                return;
            const hoje = new Date();
            ...
            else if (message.content.toLowerCase().includes("sábado") ||
            ...
            else if (message.content.toLowerCase().includes("amanha") ||
        ...
        if (command) {
            ...
            db.helped++;
            const ctx = new CommandContext_1.default(this.client, message, args);
            command.execute(ctx);
        }
    }
}
