...
class InteractionCreate {
    ...
    async run(interaction) {
        ...
        else if (interaction instanceof oceanic_js_1.CommandInteraction) {
            await this.handleCommandInteraction(interaction);
        }
        else if (interaction instanceof oceanic_js_1.ComponentInteraction) {
    ...
    async handleCommandInteraction(interaction) {
        ...
        if (cmd.category === "Music") {
            if (this.isMusicBlacklisted(db, interaction))
                return;
            if (!this.isDiscordLinked(interaction))
                return;
        }
        db.helped++;
    ...
    sendBlacklistMessage(interaction, message) {
        const embed = new this.client.embed()
            .setDescription(`:x: Ocorreu um erro ao executar esse comando!\n${message}`)
            .setColor("16711680");
        interaction.createMessage({ embeds: [embed] });
    }
    isMusicBlacklisted(db, interaction) {
    ...
    async handleSilenceInteraction(interaction) {
    ...
    async handleConfirmReadInteraction(interaction) {
        ...
        interaction.message.messageReference
            ? interaction.channel?.messages.get(interaction.message.messageReference.messageID).delete()
            : interaction.message.delete();
    }
    async handleCancelInteraction(interaction, customID) {
    ...
    async handleConfirmSoftbanInteraction(interaction, customID) {
        ...
        try {
            ...
            const embed = new this.client.embed()
                ...
                .setTimestamp();
            interaction.channel.createMessage({ content: `✅ ${interaction.member.mention} Usuário foi softbanido com sucesso.` });
            interaction.message.delete();
    ...
    async handleConfirmBanInteraction(interaction, customID) {
        const [_, __, userID, moderatorID, ...reasonParts] = customID.split("_");
        const reason = reasonParts.join("_");
        if (interaction.member.id !== moderatorID) {
            interaction.createMessage({
        ...
        try {
            ...
            const embed = new this.client.embed()
                ...
                .setThumbnail(member.user.avatarURL())
                .setTimestamp();
            interaction.channel.createMessage({ content: `✅ ${interaction.member.mention} Membro banido com sucesso.` });
            interaction.message.delete();
            const logChannel = interaction.guild.channels.get("940725594835025980");
            if (logChannel?.type === 0) {
                logChannel.createMessage({ embeds: [embed] });
    ...
    async handleConfirmMuteInteraction(interaction, customID) {
        ...
        try {
            ...
            const embed = new this.client.embed()
                ...
                .setThumbnail(member.user.avatarURL())
                .setTimestamp();
            interaction.channel.createMessage({ content: `✅ ${interaction.member.mention} Membro silenciado com sucesso.` });
            interaction.message.delete();
            const logChannel = interaction.guild.channels.get("940725594835025980");
            if (logChannel?.type === 0) {
                logChannel.createMessage({ embeds: [embed] });
    ...
    async handleConfirmKickInteraction(interaction, customID) {
        const [_, __, userID, moderatorID, ...reasonParts] = customID.split("_");
        const reason = reasonParts.join("_");
        if (interaction.member.id !== moderatorID) {
            interaction.createMessage({
        ...
        try {
            ...
            const embed = new this.client.embed()
                ...
                .setThumbnail(member.user.avatarURL())
                .setTimestamp();
            interaction.channel.createMessage({ content: `✅ ${interaction.member.mention} Membro expulso com sucesso.` });
            interaction.message.delete();
            const logChannel = interaction.guild.channels.get("940725594835025980");
            if (logChannel?.type === 0) {
                logChannel.createMessage({ embeds: [embed] });
